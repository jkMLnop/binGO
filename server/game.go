...
type Game struct {
	...
	CustomWords         bool               // True when the host has already made a word-list choice for this game — an explicit custom list (room board, host upload) or a curated profile list layered on the defaults — as opposed to the untouched server defaults. Lets the client skip the host setup lobby.
